...
echo '<div class="sc-carousel carousel-testimonial ' . $style . '" data-show="' . $show . '" data-scroll="' . $scroll . '" data-speed="' . $speed . '" data-effect="' . $effect . '">';
	...
	foreach( $recent_posts as $recent ){
		...
		if ( empty( $style ) or $style == 'style1' ) {
			...
			// Change image size depending on number of visible items
			if ( $show == '2' ) {
				$post_img = wp_get_attachment_image_src($post_id, 'column2-1/4', true);
			} else {
				$post_img = wp_get_attachment_image_src($post_id, 'column3-2/5', true);
			}
	...
	if ( empty( $style ) or $style == 'style1' ) {
		...
		echo '<div class="sc-carousel-thumbs">';
		$i = 0;
		foreach( $recent_posts as $recent ){
			$i = $i + 1;
			$post_id = get_post_thumbnail_id( $recent["ID"] );
