...
module.exports = panels.view.dialog.extend( {
	...
	render: function () {
		...
		this.$('.so-title .widget-name').closest('div[class^="so-panels-dialog"]').find('.so-content').first().addClass( 'thinkup-content-' + this.model.getWidgetField( 'id' ) ); // ThinkUpThemes customization - allows id to be added to so-content class to style ThinkUp Builder widgets. 
	...
	render: function () {
		...
		_.each( panelsOptions.widgets, function ( widget ) {
			var $w = $( this.widgetTemplate( {
				...
				id : widget.id, // TUT Customization
			} ) );
	...
	getFormValues: function ( formSelector ) {
		...
		$f.find( '[name]' ).each( function () {
			...
			try {
				...
				// Added to ensure compatibility with Caldera widgets // TUT Customization
				if( name === null){return;}
			...
			$s.find( '.so-image-selector' ).click( function ( e ) {
				...
				if ( frame === null ) {
					...
					frame.on( 'select', function () {
						...
						// Store the ID
						$s.find( 'input' ).val( attachment.url )
					} );
				}
