...
class SiteOrigin_Panels_Styles_Admin {
	...
	function render_styles_fields( $section, $before = '', $after = '', $current = array(), $post_id = 0, $args = array() ) {
		...
		foreach ( $groups as $group_id => $group ) {
			...
			?>
			<div class="style-section-wrapper <?php echo $group_id; ?>">
				...
				<div class="style-section-fields" style="display: none">
					...
					foreach ( $fields as $field_id => $field ) {
						...
						if ( $field['group'] == $group_id ) {
							?>
							<div class="style-field-wrapper <?php echo 'thinkup-style-field-' . $field['type'] . ' ' . $field_id; // TUT Customization ?>">
	...
	function render_style_field( $field, $current, $field_id ) {
		...
		switch ( $field['type'] ) {
			...
			case 'image' :
				$image = false;
				if ( ! empty( $current ) ) {
					$image = $current; // TUT Customization
				}

				?>
				<div class="so-image-selector">
					<div class="current-image" <?php if ( ! empty( $image ) ) {
						echo 'style="background-image: url(' . $image . ');"'; // TUT Customization
					} ?>>
					</div>
					...
					<input type="hidden" name="<?php echo esc_attr( $field_name ) ?>"
					       value="<?php echo $current; // TUT Customization ?>"/>
				</div>
		...
		foreach ( $fields as $k => $field ) {
			...
			switch ( $field['type'] ) {
				case 'color' :
					...
					break;
				case 'image' :
					$return[ $k ] = ! empty( $styles[ $k ] ) ? $styles[ $k ] : false; // TUT Customization
					break;
				case 'url' :
				...
				case 'measurement' :
					...
					break;
				case 'select' :
					if ( in_array( $styles[ $k ], array_keys( $field['options'] ) ) ) {
						$return[ $k ] = $styles[ $k ];
					}
					break;
				default:
