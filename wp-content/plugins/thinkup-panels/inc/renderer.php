		...
		foreach ( $layout_data as $ri => $row ) {
			...
			foreach( $row['cells'] as $ci => $cell ) {
				...
				$css->add_cell_css( $post_id, $ri, $ci, '', array(
					'width' => array(
						round( $cell['weight'] * ( 100 - ( ($cell_count - 1 ) * 2.6 ) ), 3 ) . '%',
						'calc(' . round( $cell['weight'] * ( 100 - ( ($cell_count - 1 ) * 2.6 ) ), 3 ) . '% )',
					)
				) );
			}
// START - TUT Customzation - Remove bottom margin
//			if(
...
//				}
//			}
// END - TUT Customzation - Remove responsive design
		}
...
echo '<div class="panel-grid-core">'; // TUT Customization
		foreach ( $row['cells'] as $ci => & $cell ) {
			$this->render_cell( $post_id, $ri, $ci, $cell, $row['cells'], $panels_data );
		}
echo '</div>'; // TUT Customization
