...
//						alert( 'test111-1 - ' + content );
					});
				}
			} else {
//				var content = $('iframe#widget-thinkup-builder-tinymce-c14-text_ifr').contents();
//				var content = $( '#widget-thinkup-builder-tinymce-c14-text_ifr' ).contents().find('html');
//				alert( content );
//				console.log( content );

				$( '#widget-thinkup-builder-tinymce-c14-text_ifr' ).keyup(function(){ // TUT - ID NEEDS TO BE CHANGED TO USE ID OF AN IFRAME
					var content = $( 'textarea#' + id ).attr( 'value' );
					$( 'textarea#' + id ).val( content ).change();
//					alert( 'test111-2 - ' + content ); // TUT - THIS UPDATES THE INPUT WHEN USER HITS DONE
				});
			}
		}
	}
...
//						alert( 'test222-1 - ' + content );
					}
				}
			} else {
				var content = $( 'textarea#' + id ).attr( 'value' );
				$( 'textarea#' + id ).val( content );
//				alert( 'test222-2 - ' + content );
			}
		}
	}
	...
	function thinkup_builder_open_deferred_activate_visual_editor( id ) {
		$( 'div.widget-inside:has(#' + id + ') input[id^=widget-thinkup-builder-tinymce][id$=type][value=visual]' ).each(function() {
			...
			// Otherwise wait and retry later (animation ongoing)
				...
				setTimeout(function() {
					thinkup_builder_open_deferred_activate_visual_editor( id );
					id = null;
				}, 100 );
			}
	...
	function thinkup_builder_ajax_deferred_activate_visual_editor( id ) {
		$( 'div.widget-inside:has(#' + id + ') input[id^=widget-thinkup-builder-tinymce][id$=type][value=visual]' ).each(function() {
			...
			// Otherwise wait and retry later (animation ongoing)
				...
				setTimeout(function() {
					thinkup_builder_ajax_deferred_activate_visual_editor( id );
					id=null;
				}, 100 );
			}
		});
	...
	$( document ).ready(function() {
		...
		$( document ).on( 'click', 'div.widget:has(textarea[id^=widget-thinkup-builder-tinymce]) .widget-title, div.widget:has(textarea[id^=widget-thinkup-builder-tinymce]) a.widget-action', function( event ) {
