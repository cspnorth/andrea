...
jQuery(document).ready(function($){
	...
	imageWidget = {
		...
		uploader : function( widget_id, widget_id_string ) {
			...
			var frame = wp.media({
				title : ThinkupBuilderFeatured.frame_title,
				multiple : false,
				library : { type : 'image' },
				button : { text : ThinkupBuilderFeatured.button_title }
			});
...
jQuery(window).load(function(){
	...
	jQuery('.featured-builder-urlin').each(function(){
		var imageURL = jQuery(this).val();
		jQuery(this).siblings('.thinkup_builder_featured').children('img').attr( 'src', imageURL );
		jQuery(this).closest('.uploader').siblings('.featured-builder-fields').find('.featured-builder-size').show();
	});
});
